...
```
## Original Data
```{r Parsing Demographic Data, message=FALSE}
...
setkey(patient, id)
```
### Raw number of patients ID: `r uniqueN(patient, by = 'id')`
### Distribution of patients with glomerulonephritis and vasculitis:
```{r Glomerulonephritis, message=FALSE}
...
mp_mcgn <- c('Acute nephritic syndrome, 
...
```{r Modality, error=FALSE, message=FALSE}
...
```{r Incidence of AKI, message=FALSE}
aki_plot <- aki %>% 
...
setkey(dt, id)
```
## Number of patients with available kidney function test: `r nrow(dt)`
```{r, echo=FALSE, message=FALSE}
dt <- merge(death, dt, all.y = TRUE, by = 'id')
setnames(dt, old ='date', new = 'date.death')
dt <- merge(patient, dt, all.y = TRUE, by = 'id')
```
## Patients with missing demographic information: `r sum(is.na(dt$birth.year)`
```{r, echo=FALSE, message=FALSE}
...
```
## Number of patients with pre-existing ESKD (dialysis or transplantation): `r nrow(dt[dt$preexisting_eskd == FALSE])`
...
```{r Relative Year for Melted Dataset}
...
out_dt <- out_dt %>% 
    mutate(exclude_1y = case_when(egfr_y <= 1 ~ 'yes',
                         eskd_y <= 1 ~ 'yes',
                         is.na(eskd_y) ~ 'no',
                         TRUE ~ 'no'),
...
out_dt %>% 
    group_by(egfr_freq_morethan_once) %>% count()
```
