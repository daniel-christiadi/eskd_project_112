...
```{r k-fold}
dt <- read_rds("main_dense3_dt.rds")

dt <- data.table(dt)

dt <- dcast(dt, 
      id + relyear + sex + age_init + gn_cat + time_compete +
...
error1 <- cv_performance %>% 
    ...
    labs(title = "Dense3 Error Rate (1 - Harrell's C-index) per Landmark with LOCF Prediction Horizon 5 years", 
         y = "Error Rate (%)") + 
...
brier1 <- cv_performance %>% 
    ...
    facet_grid(~ LM) + scale_fill_manual(values = c("blue", "red")) + 
    labs(title = "Dense3 Integrated Brier Score per Landmark with LOCF Prediction Horizon 5 years",
         y = "Brier Score") + 
    theme_bw() + theme(axis.title.x = element_blank(), 
                       legend.title = element_blank(),
                       legend.position = "none") +
...
(plot1 / plot2) + plot_annotation(
    title = "Dense3 ESKD vs Death VIMP"
)
```
...
vimp_rank <- rbindlist(list(vimp_eskd_rank, vimp_death_rank))
