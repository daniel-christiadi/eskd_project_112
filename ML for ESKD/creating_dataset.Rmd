...
libraries <- c('readxl', 'tidyverse', 'janitor', 'skimr', 'heatmaply', 'shiny',
...
```{r assessing outcome based on filtered ids}
...
```{r creating heatmap of data density, eval=FALSE}
...
```{r explore dataset for imputation}
dt <- meta_dt %>% 
...
test <- setDT(dt)[, lapply(.SD, function(x) mean(is.na(x))), by = id]
```
