...
libraries <- c('readxl', 'tidyverse', 'broom', 'janitor', 'outliers', 'skimr', 
               'plotly', 'heatmaply', 'shiny', 'data.table', 
               'gtsummary', 'lubridate')
...
# dt <- pathology %>% 
#     filter(test %in% c('Ca, adj', "Calcium, serum")) %>% 
#     filter(value < 10) %>% # removing likely false data 
#     pivot_wider(id_cols = c("id", "date"),
#                 names_from = test,
#                 values_from = value,
#                 values_fn = mean)
# setnames(dt, old = "Ca, adj", new = "adj_ca")
# setnames(dt, old = "Calcium, serum", new = "serum_ca")
# 
# ca <- dt %>% 
#     mutate(test = if_else(is.na(adj_ca), serum_ca, adj_ca)) %>% 
#     filter(test > 0) %>% 
#     rename(value = test) %>%
#     mutate(test = "calcium") %>% 
...
```{r compiling clinico-pathological data}
melted <- na.omit(unique(rbindlist(
    list(alb, albcreat, alkphos, alt, baso, bicarb, bili, ca_adj, chloride, 
...
We had extracted 15,433 distinct IDs while Melted had 10198 distinct IDs (based on test == "egfr"). 
...
```
There were 15 IDs without baseline demographics (sex and birthyear) and 500 IDs with pre-existing ESKD (Hx of ESKD before initial eGFR). 
```{r outcome and feature dataset}
# melted %>% 
#     filter(test == "egfr") %>% 
#     left_join(y = patient, by = "id") %>% 
#     mutate(age_init = year(date) - birth_year) %>%
#     filter(age_init < 17)
...
feature_dt <- outcome_dt %>% 
    select(id, date_egfr_init) %>% 
    right_join(y = melted, by = c("id")) %>% 
    mutate(relyear = as.duration(date - date_egfr_init)/dyears(1)) %>% 
    filter(relyear >= 0) %>% 
    select(-date_egfr_init)

# fwrite(outcome_dt, 'outcome_data.gz')
# fwrite(feature_dt, 'feature_data.gz')
...
dt <- outcome_dt %>% 
    ...
    status_eskd = if_else(
        time_eskd == eskd_fu, 1, 0, missing = 0
    )) %>% 
    mutate(time_compete = pmin(follow_up, eskd_fu, death_fu, na.rm = TRUE),
           status_compete = case_when(
               time_compete == eskd_fu ~ 1,
               time_compete == death_fu ~ 2,
               time_compete == follow_up ~ 0
           ))
# poor glucose diagnosis
poor_glucose <- feature_dt %>% 
    select(id, test, value, relyear) %>% 
    filter(test == "glucose") %>% 
    right_join(select(dt, id, time_compete), by = c("id")) %>% 
    filter(relyear <= time_compete, value >= 11.1) %>% 
    distinct(id)

poor_hba1c <- feature_dt %>% 
    select(id, test, value, relyear) %>% 
    filter(test == "hba1c") %>% 
    right_join(select(dt, id, time_compete), by = c("id")) %>% 
    filter(relyear <= time_compete, value >= 6.5) %>% 
    distinct(id)

diabetes <- poor_glucose %>% 
    bind_rows(poor_hba1c) %>% 
    distinct(id) %>% 
    mutate(dkd = "yes")

poor_systolic <- feature_dt %>% 
    select(id, test, value, relyear) %>% 
    filter(test %in% c("sitsys", "stasys")) %>% 
    right_join(select(dt, id, time_compete), by = c("id")) %>% 
    filter(relyear <= time_compete, value >= 140) %>% 
    distinct(id)

poor_diastolic <- feature_dt %>% 
    select(id, test, value, relyear) %>% 
    filter(test %in% c("sitdias", "stadias")) %>% 
    right_join(select(dt, id, time_compete), by = c("id")) %>% 
    filter(relyear <= time_compete, value >= 90) %>% 
    distinct(id)

hypertension <- poor_systolic %>% 
    bind_rows(poor_diastolic) %>% 
    distinct(id) %>% 
    mutate(htn = "yes")
    
meta_dt <- dt %>% 
    left_join(y = gn, by = c("id")) %>% 
    left_join(y = diabetes, by = c("id")) %>% 
    left_join(y = hypertension, by = c("id")) %>% 
    rename(gn = diagnosis) %>% 
    relocate(gn:htn, .after = age_init) %>% 
    replace_na(list(gn = "no", dkd = "no", htn = "no")) %>% 
    left_join(select(feature_dt, id, test, value:relyear), by = c("id")) %>% 
    relocate(relyear, .after = id) %>% 
    relocate(test:value, .after = relyear) %>% 
    select(-date_egfr_init)
...
long_dt <- meta_dt %>% 
    select(id:value) %>% 
    mutate(new_relyear = round(relyear, digits = 1)) %>% 
    select(id, new_relyear, test, value) %>% 
    rename(relyear = new_relyear) %>% 
    arrange(id, relyear) 

saveRDS(long_dt, "long_dt.RDS")
```

```{r finding IDs with reasonable data density}
test_list <- long_dt %>% 
    count(test) %>% 
    pull(test)

id_dt <- short_dt %>% 
    select(id) %>% as.data.table()
...
``` {r data density heatmap, echo=FALSE, include=FALSE eval=FALSE}
id_dt[, id := NULL]
id_dt[is.na(id_dt)] <- 0
...
```
From 9104 IDs, 4466 IDs had two or more albumin/creatinine ratio and eGFR. While, there were 7120 IDs with eGFR >= 2.
```{r dataset based on ACR>2 and eGFR>2}
acrbased_dt <- id_dt[albcreat_ratio >=2 & egfr >=2, ]
acrbased_dt[, id:=NULL]
acrbased_dt[is.na(acrbased_dt)] <- 0    

heatmap_breaks<-c(0, 1, 2, 5, 10, 20, 50, 100, 200, 500, 1000, 2000, 5000, 10000)
heatmaply(x = t(as.matrix(acrbased_dt)), ylab = 'Features', xlab = 'Patients', 
          main = 'Data Density of Clinico-Pathological Data of Patients with eGFR>=2 and ACR>=2',
          dist_method = 'manhattan', hclust_method = 'ward.D2',
          Rowv = TRUE, Colv = TRUE, dendrogram = 'both', 
          showticklabels = c(FALSE, TRUE), 
          scale_fill_gradient_fun = scale_fill_viridis(trans='sqrt', 
...
acrbased_id <- id_dt[albcreat_ratio >=2 & egfr >=2, id]

acrbased_short <- short_dt %>% 
    filter(id %in% acrbased_id)
saveRDS(acrbased_short, "acrbased_short.RDS")

acrbased_long <- long_dt %>% 
    filter(id %in% acrbased_id)
saveRDS(acrbased_long, "acrbased_long.RDS")
```
